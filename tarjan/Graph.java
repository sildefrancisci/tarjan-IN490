...
public class Graph {
	...
	private Set<Nodo> Compcon=new HashSet<Nodo>();
	private Map<Integer,Set<Nodo>> numcompcon=new HashMap<Integer, Set<Nodo>>();
	...
	public Graph() {                                   /*costruttore*/
		this.nodi= new HashMap<Nodo,Set<Arco>>();     /*Map è interfaccia non può essere istanzializzata*/
	}
	...
	public Graph(Set<Nodo> insnodi,Set<Arco> insarchi){   /*costruttore*/
		...
		while(!insnodi.isEmpty()){
			...
			Set<Arco> archi =new HashSet<Arco>();
			for(Arco a : insarchi){
	...
	public Set<Arco> archiIncidenti(Nodo v) {        /*restituisce insieme di archi incidenti in v*/
	...
	public Set<Nodo> nodiAdiacenti(Nodo v) {  /*restituisce insieme di nodi adiacenti a v */
		HashSet<Nodo> adj=new HashSet();		
	...
	public Set<Nodo> getAllNodi() {          /*restituisce i nodi*/
    		return this.nodi.keySet();
  	}

	public Nodo RandomNodo(Set<Nodo> a) {     /*restituisce un nodo random da un inseme di nodi*/
	...
	public void Visita(){		        
		if(getAllNodi().isEmpty())                                           /*controllo che il grafo non sia vuoto*/
			...
			Set<Nodo> nodes= new HashSet(getAllNodi());	
			while(!nodes.isEmpty()){                                       /*while su insieme di nodi che si modifica*/
				Set<Nodo> Compcon=new HashSet();
	...
	public Integer Visita(Nodo v,Integer i,Map<Integer,Set<Nodo>> numcompcon){ /*visito a partire dal nodo in esame*/
		HashSet<Nodo> adj=new HashSet(nodiAdiacenti(v));
	...
	public void reset(Set<Nodo> Compcon) {    /*a tutti i nodi dell'insieme si pone il marcatore come false*/
		for(Nodo v : Compcon) 
			v.setMarcatore(false);
	}

	public Integer getInt(Nodo v) {            /*restituisce l'intero della componente connessa in cui si trova il nodo v*/
		Integer i=0;
		Set<Nodo> Compcon= new HashSet<Nodo>();
	...
	public void ReCompcon(int i){    /*rimuove la i-esima componente connessa e scala le successive*/
		numcompcon.remove(i);
